...
const Navbar: React.FC<NavbarProps> = ({ onNavClick, activeSection }) => {
  ...
  return (
    <nav className={`fixed w-full z-[100] transition-all duration-700 ${
      isScrolled ? 'bg-[#2D1A1D]/90 backdrop-blur-xl py-3 shadow-2xl border-b border-white/5' : 'bg-transparent py-8'
    }`}>
          ...
          {NAV_ITEMS.map((item) => (
            <button
              ...
              className={`text-[10px] uppercase tracking-[0.3em] font-bold transition-all duration-500 relative group py-2 ${
                activeSection === item.id ? 'text-white' : 'text-[#F2BBC2]/60 hover:text-[#F2BBC2]'
              }`}
            >
              {item.label}
              <span className={`absolute bottom-0 left-0 h-[2px] bg-[#F2BBC2] transition-all duration-500 ${
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 bg-[#2D1A1D] z-[110] flex flex-col items-center justify-center transition-all duration-1000 ease-[cubic-bezier(0.85,0,0.15,1)] ${
